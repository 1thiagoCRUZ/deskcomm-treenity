...
> ```
>
> *(¿prefieres leer antes de ejecutar? el comando anterior ya clona el repositorio; el script
...
un sitio mudo. Detalles en [`setup-kit/README.md`](setup-kit/README.md).
...
Clona el repositorio en el VPS y pídele a **Claude Code**: *"instálame el DeskcommCRM"*.
Puede seguir [`setup-kit/README.md`](setup-kit/README.md)
...
**¿Salió mal?** `bash setup-kit/restore.sh` vuelve al backup.
**¿Solo quieres diagnosticar?** `bash setup-kit/healthcheck.sh`.
...
| **Hosting** | Cualquier VPS con Docker (Hostinger recomendada) | App + WhatsApp + workers en tu propia máquina |
...
| Doc | Qué tiene |
|---|---|
| [`setup-kit/README.md`](setup-kit/README.md) | **Instalación self-host** — el kit, los scripts y Hostinger |
...
- **Self-host** — `setup-kit` (app + WhatsApp + base con 1 comando), `baseline.sql` auto-curativo, **actualización desde la pantalla** con backup automático, runbook de producción.
